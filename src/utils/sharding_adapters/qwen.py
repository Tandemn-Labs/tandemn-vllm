...
class QwenShardingAdapter(ShardingAdapter):
    """Sharding adapter for Qwen (Qwen2/Qwen3) into vLLM-compatible keys.
    ...
    def shard_layer(self, layer_idx: int, hf_weights: Dict[str, torch.Tensor]) -> LayerShard:
        ...
        # Attention: fuse qkv + save q_norm/k_norm
        ...
        qnorm_key = f"{p}.self_attn.q_norm.weight"
        knorm_key = f"{p}.self_attn.k_norm.weight"
        if qnorm_key in hf_weights:
            out[qnorm_key] = hf_weights[qnorm_key].detach().cpu()
        if knorm_key in hf_weights:
            out[knorm_key] = hf_weights[knorm_key].detach().cpu()
