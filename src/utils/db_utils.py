...
def has_significant_change(current_metrics: Dict[str, Any], last_metrics: Dict[str, Any]) -> bool:
    """
    ...
    for curr_gpu, last_gpu in zip(current_gpus, last_gpus):
        ...
        if abs(curr_gpu["used_vram_gb"] - last_gpu["used_vram_gb"]) > 0.5:  # 0.5 GB threshold
            return True
        # Check utilization change
        if (curr_gpu["utilization_percent"] is not None and 
            last_gpu["utilization_percent"] is not None and
            abs(curr_gpu["utilization_percent"] - last_gpu["utilization_percent"]) > 5):
            return True
        # Check temperature change
        if (curr_gpu["temperature_celsius"] is not None and 
            last_gpu["temperature_celsius"] is not None and
            abs(curr_gpu["temperature_celsius"] - last_gpu["temperature_celsius"]) > 2):
            return True
    ...
    cursor = db[METRICS_COLLECTION].find(
        {
            "peer_id": peer_id,
            "timestamp": {
                "$gte": datetime.utcnow() - timedelta(seconds=time_window)
            }
        },
        sort=[("timestamp", -1)]
    )
